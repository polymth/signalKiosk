...
def ReadTelemList ():
	print('\nAttempting to READ telem_list from FILE!\n')
	return
...
def newGetSignalkValue(sks,tlm,skval):
...
def GetHeading ():
	...
	skstring = "ap.heading"
	dispVal=GetSignalkValue(skstring) #GetSignalkValue(sks,tlm,skval)
...
def callGsKVProc(sks,tlm,skval):
	
	#TEST COMMUNICATION TO SUB-PROCESS_def GsKVProc()
	##############################################################
	skval.send(sks)
	...
	if skval.poll():
		it = skval.recv()
		print('\nTelemetry Returned From SK-PROCESS: "' + it + '"\n')
		#return it
		...
		if connection:
			...
			while True:
				...
				if skval.poll():
					...
					msg = kjson.loads(line.rstrip())
					value = msg[wsk]["value"]
					#print('Proc_Telemetry value for ' + wsk + '\n')
...
def main():
	...
	while True:
		...
		#pip3 install keyboard module
		#if keyboard.is_pressed('S') or keyboard.is_pressed('s'):
			#then do Button_Callback()
