...
telem_list = ['TIME','ALL','BNAV','HDG','CSE','HEEL','TEST']
...
def DispPUSH ():
	...
	segment.set_digit_raw(1,0xBE)
	segment.set_digit_raw(2,0xED)
	segment.set_digit_raw(3,0xF6)
	segment.write_display()
	time.sleep(.5)
	segment.clear()
...
def Button_Callback (pin):
	...
	DispPUSH ()
	DISPLAYCMD = bcProxy()  #***********THIS LINE WAS NOT BEING COMPLETED*******************
	return DISPLAYCMD
...
def main():
	...
	DISPLAYCMD		= 'BNAV'
	DISPLAYLABEL	= 0
	LABELINTERVAL	= 2000 #15000 milliseconds = 15 seconds
	...
	#(tlm,skval) = Pipe(False) #into GetSignalkValue()
	#(skvaldos,tlmdos) = Pipe(False) #from GetSignalkValue()
	#proc = Process(target=GetSignalkValue, args=(skval,tlm))
	#proc.start()
